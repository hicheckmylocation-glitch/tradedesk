...
async function fetchGoldpetalAuthorizedQuote() {
  ...
  const price = parseNumber(getPathValue(data, pricePath));
  if (!price || price <= 0) return null;

  const prev = parseNumber(getPathValue(data, prevPath));
  const change = parseNumber(getPathValue(data, changePath));
  return {
    price,
